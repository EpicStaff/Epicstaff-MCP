...
const surfaceToolEntrySchema = z
  .union([
    symbolicNameSchema.transform((name): SurfaceToolEntry => ({ tool: name, mode: 'allow' })),
    existingRefSchema.transform((ref): SurfaceToolEntry => ({ tool: ref, mode: 'allow' })),
    surfaceToolObjectSchema,
  ])
  .describe(
    'A tool grant. Shorthand: a bare reference means {tool: <ref>, mode: "allow"}. Use {tool, mode: "deny"} to hard-deny.',
  );

export const storageAccessSchema = z
  .enum(['allow', 'unset', 'deny'])
  .describe(
    'Tri-state storage permission. When surfaces are combined: deny > allow > unset.',
  );
...
export const surfaceStorageItemSchema = z
  .strictObject({
    file: z
      ...
      .min(1)
      .describe(
        'Remote storage file or folder path, e.g. "reports/summary.md". Resolved against org storage at push time.',
      ),
...
export type SurfaceStorageItemSource = z.infer<typeof surfaceStorageItemSchema>;

export interface SurfaceKnowledgeEntry {
  collection: EntityRef;
  naive_config?: Record<string, unknown>;
  graph_basic_config?: Record<string, unknown>;
  ...
  .superRefine((entry, ctx) => {
    ...
    if (configCount > 1) {
      ctx.addIssue({
        ...
        message:
          'at most one of naive_config / graph_basic_config / graph_local_search_config may be set',
      });
    }
  });
...
const surfaceKnowledgeEntrySchema = z
  .union([
    symbolicNameSchema.transform((name): SurfaceKnowledgeEntry => ({ collection: name })),
    existingRefSchema.transform((ref): SurfaceKnowledgeEntry => ({ collection: ref })),
    surfaceKnowledgeObjectSchema,
  ])
  .describe(
    'A knowledge grant. Shorthand: a bare reference exposes the collection with its default search config.',
  );
...
export const inlineSurfaceSchema = z
  ...
  .describe(
    'Ad-hoc surface embedded in a node. Same shape as a catalog surface but unnamed, not shareable, and deleted with the node.',
  );

export type InlineSurfaceSource = z.infer<typeof inlineSurfaceSchema>;
...
export const catalogSurfaceSchema = z
  .strictObject({
    ...
    owner_agent: entityRefSchema
      .optional()
      .describe(
        'When set, this surface is agent-specific: only the owning agent may attach it. Omit for a shared surface.',
      ),
  })
  .describe('One named catalog surface.');

export type CatalogSurfaceSource = z.infer<typeof catalogSurfaceSchema>;
