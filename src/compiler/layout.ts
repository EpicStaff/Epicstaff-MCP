...
export interface LayoutNode {
    id: string;
    /** Node type string — must match the frontend NodeType enum values (see LAYOUT_NODE_TYPES). */
    type: string;
    /** Rendered size. Defaults to 330 x 60 when absent (frontend fallback values). */
    size?: LayoutSize;
}
...
export interface LayoutConnection {
    ...
    targetNodeId: string;
    /**
     * Frontend CustomPortId format: '<nodeId>_<portRole>'. The role after the
     * first underscore drives port-order child sorting (Decision-Table lanes).
     */
    sourcePortId?: string;
    targetPortId?: string;
}
...
function nHeight(n: LayoutNode | undefined): number {
    return n?.size?.height ?? 60;
}

function nWidth(n: LayoutNode | undefined): number {
 ...
 */
function buildUndirectedComponents(nodes: LayoutNode[], connections: LayoutConnection[]): string[][] {
...
function layoutSingleComponent(
    ...
    startY: number
): { positions: Map<string, LayoutPoint>; bottomY: number } {
