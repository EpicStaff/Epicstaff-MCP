...
import type { AgentNodeTaskUi, AgentNodeTaskWrite } from '../models/nodes/agent-node.js';
import type {
  CdtConditionGroupWrite,
  ClassificationDecisionTableNodeWrite,
  PromptConfigWrite,
} from '../models/nodes/classification-decision-table-node.js';
import type { DecisionTableConditionGroupWrite, DecisionTableNodeWrite } from '../models/nodes/decision-table-node.js';
import type { BulkEdgeCreate, BulkEdgeUpdate } from '../models/nodes/edge.js';
import type { ScheduleBlockWrite } from '../models/nodes/schedule-trigger-node.js';
import { buildUuidToBackendIdMap, getConnectionDiff, getNodeDiff, type NodeDiff } from './diff.js';
...
function buildDecisionTableNodePayload(
  node: DecisionTableGraphNode,
  graphId: number,
  allNodes: GraphNode[],
  idMap: Map<string, number>
): DecisionTableNodeWrite {
  const tableData = node.data.table;
  const conditionGroups: DecisionTableConditionGroupWrite[] = tableData.condition_groups
...
function buildScheduleBlock(node: ScheduleTriggerGraphNode): ScheduleBlockWrite {
  ...
  let end: ScheduleBlockWrite['end'];
...
function serializeCdtFieldExpressions(fieldExpressions: Record<string, unknown>): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(fieldExpressions)) {
    if (typeof value === 'object' && value !== null && 'operator' in value) {
      const expression = value as { field?: string; operator?: string; value?: unknown };
...
function buildCdtNodePayload(
  node: ClassificationDecisionTableGraphNode,
  graphId: number,
  allNodes: GraphNode[],
  idMap: Map<string, number>,
  ...
  const conditionGroups: CdtConditionGroupWrite[] = [...(tableData?.condition_groups || [])]
    .sort((a, b) => (a.order ?? Number.MAX_SAFE_INTEGER) - (b.order ?? Number.MAX_SAFE_INTEGER))
    .map((group, index) => {
      ...
      if (group.route_code) {
        ...
        if (!targetUuid) {
          ...
          const edge = edges.find(
            (candidate) => candidate.sourceNodeId === node.id && candidate.sourcePortId === routePortId
          );
          if (edge) targetUuid = edge.targetNodeId;
        }
      }
  ...
  if (!defaultTargetUuid) {
    const edge = edges.find(
      (candidate) => candidate.sourceNodeId === node.id && candidate.sourcePortId === `${node.id}_decision-default`
    );
    if (edge) defaultTargetUuid = edge.targetNodeId;
  }

  let errorTargetUuid: string | null = tableData?.next_error_node ?? null;
  if (!errorTargetUuid) {
    const edge = edges.find(
      (candidate) => candidate.sourceNodeId === node.id && candidate.sourcePortId === `${node.id}_decision-error`
    );
    if (edge) errorTargetUuid = edge.targetNodeId;
  }
...
function nodeItems<TNode extends { id: string; backendId: number | null }, TWrite extends object>(
  diff: NodeDiff<TNode>,
  mapPayload: (node: TNode) => TWrite
): BulkItem<TWrite>[] {
  const created = diff.toCreate.map(
    (node) => ({ id: null, temp_id: node.id, ...mapPayload(node) }) as BulkCreateItem<TWrite>
  );
  const updated = diff.toUpdate.map(
    ({ current: node }) => ({ id: node.backendId as number, ...mapPayload(node) }) as BulkUpdateItem<TWrite>
  );
  return [...created, ...updated];
}

export interface BuildBulkSavePayloadOptions {
  /** Backend id of the graph being saved. */
  graphId: number;
  /** The desired (local) graph state to persist. */
  desired: GraphState;
  /** The last-known persisted state — the diff baseline. */
  remote: GraphState;
  /** Optimistic-concurrency version from the last `GraphDto`. */
  saveVersion: number;
}

export function buildBulkSavePayload(options: BuildBulkSavePayloadOptions): BulkSavePayload {
  ...
  const edgeList: BulkEdgeCreate[] = connectionDiff.toCreate.map((edge) => {
  ...
  const edgeUpdateList: BulkEdgeUpdate[] = connectionDiff.toUpdate.map((edge) => {
    const startNodeId = idMap.get(edge.sourceNodeId);
    const endNodeId = idMap.get(edge.targetNodeId);
    return {
      id: edge.backendId!,
  ...
  return {
    ...
    decision_table_node_list: nodeItems(nodeDiff.decisionTableNodes, (node) =>
      buildDecisionTableNodePayload(node, graphId, desired.nodes, idMap)
    ),
