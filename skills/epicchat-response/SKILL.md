...
Tool toggles shown in the input area dropdown. Include `["Build mode"]` in every response from a build-capable agent to keep the build/plan toggle visible. Omit for plan-only agents or when no tool toggles are configured.
...
**With options** — explicit columns, sorting, selection:
```json
{
  "ef_tables": [{
    "columns": [
...
**Combining `type` and `action`:** For `type: button` — choose an `action` from the table below. For `type: link` — always set `action: link`. For `type: prompt` — no `action` needed.
...
**When NOT to use:** Content written to files on disk (e.g. `spec.md`, `flow_arch.md`) — only the agent's chat responses back to the user need this format.

**Common mistakes:**
- Wrapping the JSON output in a markdown code fence — EpicChat expects raw JSON, not ` ```json ` blocks
- Including fields with null or empty-array values — omit them entirely
- Using `processTables` without a table that has `rowsSelectionType: "select"` or `"multiSelect"`
- Emitting `refreshCache` without an accompanying `openFlow` or `openNode` button
- Forgetting `["Build mode"]` in `tools` on a build-capable agent, causing the toggle to disappear
