...
| Decision Table (DT) | `decisiontablenode` | `condition_groups[]`, `default_next_node_id`, `next_error_node_id` | `input` (input) | `decision-default`, `decision-error`, `decision-out-{group_name}` | **Avoid — prefer CDT.** Routing is metadata-only (`add_edge` does nothing); wire via `patch_dt_node`. Route by integer `next_node_id` — a stray `next_node` NAME is now rejected client-side with a 400 by `patch_dt_node` (it used to reach the backend and 500). |
...
| Webhook Trigger | `webhooktriggernode` | `webhook_trigger.path` (nested — a flat `webhook_path` is silently dropped), `python_code.code` | none | `webhook-trigger-out` (single) | No input port — nothing wires TO it. Also connect `__start__` to same downstream node for manual runs. |
