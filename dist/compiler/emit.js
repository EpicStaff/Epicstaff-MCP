 ...
 */
class RefRegistry {
    existingBySection = new Map();
    ref(section, entityRef) {
...
async function readFlowFile(flowDir, relativePath) {
    return fs.readFile(path.resolve(flowDir, relativePath), 'utf8');
}
// ---------------------------------------------------------------------------
// Entity plans
// ---------------------------------------------------------------------------
function upsertPlan(section, name, kind, sourceDefinition, payload, extra) {
    return {
...
function buildLlmConfigPlans(source) {
    return Object.entries(source.llm_configs).map(([name, config]) => {
        ...
        const apiKey = config.api_key_env !== undefined ? { $env: config.api_key_env } : '';
        const payload = {
            custom_name: name,
...
function buildToolConfigPlans(source) {
...
async function buildPythonToolPlans(source, flowDir, diagnostics) {
    const plans = [];
    for (const [name, tool] of Object.entries(source.tools.python_code_tools)) {
        const code = tool.code ?? (await readFlowFile(flowDir, tool.code_file));
        if (tool.args_schema !== undefined) {
            diagnostics.push(makeWarning(`tools.python_code_tools.${name}.args_schema`, 'args_schema is not supported by the python-code-tool create API and is ignored'));
        }
        plans.push(upsertPlan('tools.python_code_tools', name, 'python_code_tool', tool, {
            name,
...
function buildMcpToolPlans(source, diagnostics) {
    return Object.entries(source.tools.mcp_tools).map(([name, tool]) => {
        if (tool.url === undefined) {
            diagnostics.push(makeWarning(`tools.mcp_tools.${name}.command`, 'stdio MCP tools cannot be pushed — the backend McpTool.transport field only accepts a remote server URL; this plan will fail at push time'));
        }
...
function buildKnowledgePlans(source, flowDir, registry) {
    return Object.entries(source.knowledge).map(([name, collection]) => upsertPlan('knowledge', name, 'knowledge_collection', collection, { collection_name: name }, {
        documents: collection.documents.map((documentPath) => path.resolve(flowDir, documentPath)),
        rag: buildRagPlan(collection, registry),
    }));
}
function buildRagPlan(collection, registry) {
    const rag = collection.rag;
    if (rag.strategy === 'naive') {
        return {
            strategy: 'naive',
            embedder: rag.embedder !== undefined
...
function buildSurfacePlans(source, registry) {
    return Object.entries(source.surfaces).map(([name, surface]) => upsertPlan('surfaces', name, 'surface', surface, {
        name,
        ...(surface.description !== undefined ? { description: surface.description } : {}),
        instructions: surface.instructions,
        owner_agent: surface.owner_agent !== undefined ? registry.ref('agents', surface.owner_agent) : null,
        ...
        storage_items: surface.storage.map((item) => ({
            ...
            storage_file: { $storageFile: item.file },
...
function buildAgentPlans(source, registry) {
    return Object.entries(source.agents).map(([name, agent]) => upsertPlan('agents', name, 'agent_definition', agent, {
        name,
...
function mapInlineSurface(surface, basePath, registry, diagnostics) {
    surface.storage.forEach((item, index) => {
        const unsupported = ['can_list', 'can_edit', 'can_delete'].filter((flag) => item[flag] !== 'unset');
        if (unsupported.length > 0) {
            diagnostics.push(makeWarning(`${basePath}.storage[${index}]`, `inline surfaces only support can_view — ${unsupported.join(', ')} ignored`));
        }
    });
    return {
        instructions: surface.instructions,
        ...
        knowledge: surface.knowledge.map((entry, index) => {
            if (entry.graph_basic_config !== undefined || entry.graph_local_search_config !== undefined) {
                diagnostics.push(makeWarning(`${basePath}.knowledge[${index}]`, 'inline surfaces only support naive_search_config — graph search configs ignored'));
            }
            return {
                collection: registry.ref('knowledge', entry.collection),
                ...(entry.naive_config !== undefined ? { naive_search_config: entry.naive_config } : {}),
...
async function buildGraph(source, flowDir, registry, diagnostics) {
    ...
    const pushEdge = (edge, layoutSourcePortId) => {
    ...
    for (const [nodeName, node] of Object.entries(source.flow.nodes)) {
        ...
        const base = {
            ...
            nodeNumber,
            input_map: ('input_map' in node ? node.input_map : {}),
            output_variable_path: 'output_variable_path' in node && node.output_variable_path !== undefined
        ...
        switch (node.type) {
            ...
            case 'agent': {
                const data = {
                    ...
                    agent_definition: registry.ref('agents', node.agent),
                    surface_list: node.surfaces.map((ref) => registry.ref('surfaces', ref)),
            ...
            case 'task': {
                const instructions = node.expected_output !== undefined
            ...
            case 'python': {
                const code = node.code ?? (await readFlowFile(flowDir, node.code_file));
                nodes.push({
                    ...base,
                    type: 'python',
            ...
            case 'file-extractor':
                if (node.file !== undefined) {
                    diagnostics.push(makeWarning(`${nodePath}.file`, "the canvas file-extractor node has no 'file' slot yet — supply the file via input_map instead"));
                }
                nodes.push({ ...base, type: 'file-extractor' });
                break;
            case 'audio-to-text':
                if (node.model !== undefined) {
                    diagnostics.push(makeWarning(`${nodePath}.model`, "the canvas audio-to-text node has no 'model' slot — the backend default transcription model is used"));
                }
            ...
            case 'subgraph':
                nodes.push({
                    ...
                    data: { id: registry.ref('flows', node.graph) },
                });
                break;
            case 'crew': {
                ...
                nodes.push({
                    ...base,
                    type: 'crew',
                    data: { id: registry.ref('crews', node.crew) },
                });
                break;
            }
            ...
            case 'telegram-trigger':
                nodes.push({
                    ...
                    data: {
                        ...
                        telegram_bot_api_key: node.bot_token_env !== undefined
                            ? { $env: node.bot_token_env }
                            : '',
                        webhook_trigger: null,
                        fields: [],
                    },
            ...
            case 'schedule-trigger':
                diagnostics.push(makeWarning(`${nodePath}.schedule`, `cron expression '${node.schedule}' cannot be translated to the backend schedule block yet — the node is created as an inactive draft; configure the schedule in the UI`));
                nodes.push({
                    ...base,
                ...
                nodes.push({
                    ...
                    data: {
                        ...
                        table: {
                            ...
                            default_next_node: node.default_next_node !== undefined ? uuidOf(node.default_next_node) : null,
            ...
            case 'classification-decision-table': {
                if (node.categories.some((category) => category.description !== undefined)) {
                    diagnostics.push(makeWarning(`${nodePath}.categories`, 'category descriptions are not yet mapped into the classification table state and are ignored'));
                }
                nodes.push({
                    ...base,
                    ...
                    data: {
                        ...
                        table: {
                            ...
                            prompts: {},
                            default_llm_config: registry.ref('llm_configs', node.llm_config),
                            default_next_node: node.default_next_node !== undefined ? uuidOf(node.default_next_node) : null,
                            next_error_node: null,
                        },
                    },
    ...
    for (const edge of source.flow.edges) {
        if (edge.condition !== undefined) {
            const code = edge.condition.code ?? (await readFlowFile(flowDir, edge.condition.code_file));
        ...
        pushEdge({ sourceNodeId: uuidOf(edge.from), targetNodeId: uuidOf(edge.to) });
    }
...
// Layout
// ---------------------------------------------------------------------------
function applyLayout(source, nodes, layoutConnections) {
 ...
 */
export async function emitFlow(source, flowDir) {
    const diagnostics = [];
    ...
    const { nodes, edges, layoutConnections, conditionalEdges } = await buildGraph(source, flowDir, registry, diagnostics);
