/**
 * Standalone port of the EpicStaff frontend auto-layout algorithm.
 *
 * Source of truth:
 ...
 * replaced by minimal structural input types.
 */
// ── Node type strings (identical to the frontend NodeType enum values) ─────
...
// Grid cell size used by snapToGrid (node-placement.utils.ts)
...
export const DT_EXTRA_HORIZONTAL_GAP = 100;
...
export const DISCONNECTED_MARGIN = 250;
// Vertical gap between stacked connected components
export const COMPONENT_VERTICAL_GAP = 500;
 ...
 */
function getPortSortKey(portRole) {
    ...
    if (portRole === 'decision-default')
        return 100_000;
    if (portRole === 'decision-error')
        return 100_001;
    return 0;
}
function nHeight(n) {
 ...
 */
function buildUndirectedComponents(nodes, connections) {
 ...
 */
function layoutSingleComponent(componentNodeIds, nodeMap, connections, startY) {
    ...
    for (const conn of connections) {
        if (!componentSet.has(conn.sourceNodeId) || !componentSet.has(conn.targetNodeId))
            continue;
    ...
    const triggerTypes = new Set([
        LAYOUT_NODE_TYPES.START,
        LAYOUT_NODE_TYPES.WEBHOOK_TRIGGER,
        LAYOUT_NODE_TYPES.TELEGRAM_TRIGGER,
    ]);
    ...
    const ownedChildren = new Map(); // primary parent → children in port order
    const primaryParent = new Map(); // child → owning parent id
    const allParents = new Map(); // child → all parents (for merge centering)
    // Port role of the source connection for each child — used for crossing-free ordering
    const childSourcePortRole = new Map();
    ...
    for (const conn of connections) {
        ...
        if (srcL === undefined || tgtL === undefined || srcL >= tgtL)
            continue;
        ...
        if (!primaryParent.has(conn.targetNodeId)) {
            ...
            const sep = conn.sourcePortId?.indexOf('_') ?? -1;
            if (sep !== -1)
                childSourcePortRole.set(conn.targetNodeId, conn.sourcePortId.slice(sep + 1));
        }
    }
    ...
    for (const [, children] of ownedChildren) {
        if (children.length < 2)
            continue;
        children.sort((a, b) => getPortSortKey(childSourcePortRole.get(a) ?? '') - getPortSortKey(childSourcePortRole.get(b) ?? ''));
    }
    ...
    for (const nodeId of [...bfsOrder].reverse()) {
        ...
        if (children.length === 0) {
            ...
            const childrenTotal = children.reduce((sum, cid) => sum + (subtreeSpan.get(cid) ?? 60), 0) +
    ...
    for (const layer of sortedLayers) {
        ...
        const hasDT = ids.some((id) => nodeMap.get(id)?.type === LAYOUT_NODE_TYPES.TABLE);
 ...
 */
export function computeAutoArrangePositions(nodes, connections) {
    const nodeMap = new Map(nodes.map((n) => [n.id, n]));
    const nonNoteNodes = nodes.filter((n) => n.type !== LAYOUT_NODE_TYPES.NOTE);
