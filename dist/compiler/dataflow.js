...
export function validateDataflow(source) {
    const diagnostics = [];
    ...
    function checkReads(inputMap, readerNode, basePath) {
        for (const [key, rawValue] of Object.entries(inputMap)) {
            ...
            if (parsed.isShared || parsed.hasDefault)
                continue;
            ...
            if (producedElsewhere) {
                diagnostics.push(makeWarning(readPath, `reads 'variables.${read.join('.')}' which is produced by '${producedElsewhere.node}', ` +
                    `but no path from it reaches '${readerNode}' — it may be unset at runtime.`));
            }
            else {
                diagnostics.push(makeError(readPath, `reads 'variables.${read.join('.')}' which no node produces and isn't declared. ` +
                    `Declare it under 'variables:' (with a default), add a '|default' to the read, ` +
                    `or produce it upstream via output_variable_path.`));
            }
        }
    }
    function ancestorProduced(readerNode) {
        const result = [];
        for (const ancestor of ancestors.get(readerNode) ?? new Set()) {
...
function startInitialStateKeys(source) {
    for (const node of Object.values(source.flow.nodes)) {
        if (node.type === 'start') {
            return Object.keys(node.initial_state ?? {});
        }
    }
    return [];
}
 ...
 */
function buildAncestors(source) {
