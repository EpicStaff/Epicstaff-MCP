...
export function resolveFlow(source) {
    const diagnostics = [];
    const references = [];
    const symbols = {
    ...
    function resolveEntity(ref, section, path) {
        ...
        const otherSection = Object.keys(symbols).find((candidate) => candidate !== section && symbols[candidate].includes(ref));
        if (otherSection !== undefined) {
            diagnostics.push(makeError(path, `type mismatch: '${ref}' is defined in section '${otherSection}', but this position expects a ${SECTION_LABELS[section]}`));
            return;
        }
        diagnostics.push(makeError(path, `unresolved reference '${ref}' — ${knownNamesHint(section, symbols[section])}. ` +
            `Use { existing: "${ref}" } to reference an entity that already exists on the backend.`));
    }
    function resolveNodeName(name, path) {
        if (symbols.nodes.includes(name)) {
            references.push({ path, section: 'nodes', ref: { kind: 'local', name } });
            return;
        }
        diagnostics.push(makeError(path, `unknown node '${name}' — ${knownNamesHint('nodes', symbols.nodes)}`));
    }
    function resolveSurfaceBody(surface, basePath) {
    ...
    for (const [collectionName, collection] of Object.entries(source.knowledge)) {
        if (collection.rag.strategy === 'graph' && collection.rag.llm_config !== undefined) {
            resolveEntity(collection.rag.llm_config, 'llm_configs', `knowledge.${collectionName}.rag.llm_config`);
        }
    }
    // --- catalog surfaces ----------------------------------------------------
        ...
        switch (node.type) {
            ...
            case 'subgraph': {
                ...
                if (typeof node.graph !== 'string') {
                    ...
                    diagnostics.push(makeError(refPath, `circular subgraph reference: '${node.graph}' is this flow itself`));
                }
                else {
...
export function findResolution(resolved, path) {
    return resolved.references.find((reference) => reference.path === path)?.ref;
}
function knownNamesHint(section, known) {
    if (known.length === 0) {
