...
export const llmConfigSchema = z
    ...
    provider: z
        .string()
        .optional()
        .describe('LLM provider, e.g. "openai", "anthropic", "groq". When omitted the backend infers it from the model name.'),
    temperature: z
        .number()
        .min(0)
        .max(2)
    ...
    api_key_env: z
        .string()
        .optional()
        .describe('Name of the environment variable that holds the provider API key. The key value itself never lives in flow source.'),
    params: z
