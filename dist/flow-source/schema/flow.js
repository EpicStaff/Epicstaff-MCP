 ...
 */
import { z } from 'zod';
import { entityRef, entityRefSchema, existingRefSchema, inputMapSchema, positionSchema, symbolicNameSchema, } from './common.js';
...
export const FORBIDDEN_NODE_TYPES = ['llm', 'code-agent'];
...
export const agentNodeSchema = z.strictObject({
    ...
    tasks: z
        ...
        .default([])
        .describe('Ordered tasks the agent executes at this node. The runtime requires at least one — ' +
...
export const pythonNodeSchema = z.strictObject({
    ...
    code_file: z
        .string()
        .optional()
        .describe('Path to a Python file relative to the flow directory. Provide exactly one of code / code_file.'),
    entrypoint: z
        .string()
        .default('main')
...
export const subgraphNodeSchema = z.strictObject({
    ...
    position: positionField,
    graph: entityRef('The flow embedded by this node. A local name refers to a sibling flow directory (resolved at build time); referencing this flow itself is a circular-reference error.'),
    input_map: inputMapField,
    output_variable_path: outputVariablePathField,
});
...
export const edgeConditionSchema = z
    ...
    code: z
        .string()
        .optional()
        .describe('Inline Python decision code: the entrypoint receives the mapped inputs and returns the name of the next node. Provide exactly one of code / code_file.'),
    code_file: z
        .string()
        .optional()
        .describe('Path to a Python file relative to the flow directory. Provide exactly one of code / code_file.'),
