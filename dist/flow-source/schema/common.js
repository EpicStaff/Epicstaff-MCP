...
export const symbolicNameSchema = z
    .string()
    .regex(SYMBOLIC_NAME_PATTERN, 'symbolic names must start with a letter or underscore and may contain only letters, digits, underscores and dashes')
    .describe('Symbolic name of an entity defined in this flow source (a key of the corresponding section).');
/** Reference to an entity that already exists on the EpicStaff backend. */
export const existingRefSchema = z.strictObject({
    existing: z
        .string()
        .min(1)
        .describe('Name of an entity that already exists on the EpicStaff backend. It is reused as-is instead of being created from this flow source.'),
});
...
export function entityRef(description) {
    return z
...
export const positionSchema = z
    ...
    .describe('Optional canvas position pin. Layout is normally computed automatically; set this only to pin one node in place.');
export const inputMapSchema = z
    .record(z.string(), z.string())
    .describe('Maps this node\'s named inputs to flow-state variable paths, e.g. { "query": "variables.user_query" }.');
