...
import { z } from 'zod';
import { hasErrors, joinPath, makeError, makeWarning, } from './diagnostics.js';
...
export async function loadFlowDirectory(flowDir) {
    ...
    if (fileNames.length === 0) {
        diagnostics.push(makeError('', 'no flow source files found — expected flow.yaml (or flow.yml / flow.json) or *.flow.yaml split files'));
        return { source: null, diagnostics, files: [] };
    }
    const merged = {};
    ...
    for (const fileName of fileNames) {
        ...
        if (!isPlainObject(data)) {
            diagnostics.push(makeError('', 'top level of a flow source file must be a mapping of sections', fileName));
            continue;
        }
...
async function discoverSourceFiles(flowDir) {
    const entries = await fs.readdir(flowDir, { withFileTypes: true });
    return entries
        .filter((entry) => entry.isFile() && (ROOT_FILE_PATTERN.test(entry.name) || SPLIT_FILE_PATTERN.test(entry.name)))
...
// ---------------------------------------------------------------------------
function mergeFile(merged, provenance, fileName, data, diagnostics) {
    for (const [section, value] of Object.entries(data)) {
        if (section === 'meta') {
            mergeNamedMap(merged, provenance, fileName, 'meta', value, diagnostics, 'meta key');
        }
        else if (NAMED_MAP_SECTIONS.includes(section)) {
...
function mergeNamedMap(merged, provenance, fileName, sectionPath, value, diagnostics, duplicateNoun) {
    ...
    for (const [name, entity] of Object.entries(value)) {
        ...
        if (Object.prototype.hasOwnProperty.call(target, name)) {
            ...
            diagnostics.push(makeError(entryPath, `duplicate ${duplicateNoun} '${name}' in section '${sectionPath}'${firstFile !== undefined ? ` — already defined in ${firstFile}` : ''}`, fileName));
            continue;
        }
        target[name] = entity;
...
function mergeToolsSection(merged, provenance, fileName, value, diagnostics) {
    if (!isPlainObject(value)) {
    ...
    for (const [key, subValue] of Object.entries(value)) {
        if (TOOL_SUBSECTIONS.includes(key)) {
...
function mergeFlowSection(merged, provenance, fileName, value, diagnostics) {
    if (!isPlainObject(value)) {
...
function mergeOpaque(target, provenance, fileName, fullPath, value, diagnostics) {
    const key = lastPathSegment(fullPath);
    if (Object.prototype.hasOwnProperty.call(target, key)) {
        const firstFile = provenance.get(fullPath);
        diagnostics.push(makeError(fullPath, `'${fullPath}' is defined in more than one file${firstFile !== undefined ? ` — already defined in ${firstFile}` : ''}`, fileName));
        return;
    }
    target[key] = value;
 ...
 */
function getOrCreateObject(parent, provenance, fileName, sectionPath, diagnostics) {
    ...
    diagnostics.push(makeError(sectionPath, `'${sectionPath}' is defined in more than one file with incompatible shapes${firstFile !== undefined ? ` — already defined in ${firstFile}` : ''}`, fileName));
    return null;
}
// ---------------------------------------------------------------------------
// zod issue → diagnostic
// ---------------------------------------------------------------------------
function issueToDiagnostic(issue, provenance) {
...
function lookupProvenance(provenance, segments) {
...
function nodeFile(provenance, nodeName) {
...
// ---------------------------------------------------------------------------
function collectNodeTypeDiagnostics(source, provenance) {
    const diagnostics = [];
    for (const [nodeName, node] of Object.entries(source.flow.nodes)) {
        ...
        if (node.type === 'llm') {
            diagnostics.push(makeError(`${nodePath}.type`, "node type 'llm' is legacy and can no longer be created — use an 'agent' node instead", file));
        }
        else if (node.type === 'code-agent') {
            diagnostics.push(makeError(`${nodePath}.type`, "node type 'code-agent' is legacy and can no longer be created — use an 'agent' node with a python tool instead", file));
        }
        else if (node.type === 'crew') {
            diagnostics.push(makeWarning(`${nodePath}.type`, "node type 'crew' is deprecated — prefer 'agent' and 'task' nodes", file));
        }
...
// ---------------------------------------------------------------------------
async function collectLocalFileDiagnostics(source, provenance, flowDir) {
    const diagnostics = [];
    const check = async (relativePath, atPath, file) => {
        if (path.isAbsolute(relativePath) || path.normalize(relativePath).startsWith('..')) {
            diagnostics.push(makeError(atPath, `'${relativePath}' must be a relative path inside the flow directory`, file));
            return;
        }
        try {
            ...
            diagnostics.push(makeError(atPath, `file not found: '${relativePath}' (resolved against the flow directory)`, file));
        }
    };
...
function lastPathSegment(sectionPath) {
