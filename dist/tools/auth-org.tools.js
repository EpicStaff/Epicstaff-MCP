...
export async function runTool(work) {
    try {
        return toContent(ok(await work()));
    }
    catch (error) {
        if (error instanceof ApiError) {
            return toContent(err(error.message, {
...
export function registerAuthOrgTools(server, context) {
    server.registerTool('check_connection', {
        title: 'Check EpicStaff connection',
        description: 'Verify connectivity and authentication with the EpicStaff backend: validates or mints the API key ' +
        ...
        return {
            ...
            ...(orgStatus.selectionRequired && {
                actionRequired: 'Multiple organizations available — call set_active_organization with one of the listed ids before creating anything.',
            }),
        };
    }));
    server.registerTool('list_organizations', {
        title: 'List organizations',
        description: 'List the organizations the configured user belongs to, and which one is currently active. ' +
    ...
    server.registerTool('set_active_organization', {
        title: 'Set active organization',
        description: 'Select the active organization. All subsequent API calls carry it as X-Organization-Id; ' +
    ...
    }, async ({ organization_id }) => runTool(async () => {
        await context.auth.ensureAuthenticated();
