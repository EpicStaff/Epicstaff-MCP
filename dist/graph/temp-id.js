 ...
 */
export function applySaveResponse(desired, remote, response) {
    const mapping = new Map();
    const nodeDiff = getNodeDiff(remote, desired);
    const existingIdsByType = (type) => new Set(remote.nodes
    ...
    mapByNewIds(nodeDiff.fileExtractorNodes.toCreate, response.file_extractor_node_list ?? [], existingIdsByType('file-extractor'));
    mapByNewIds(nodeDiff.audioToTextNodes.toCreate, response.audio_transcription_node_list ?? [], existingIdsByType('audio-to-text'));
    mapByNewIds(nodeDiff.subgraphNodes.toCreate, response.subgraph_node_list ?? [], existingIdsByType('subgraph'));
    mapByNewIds(nodeDiff.webhookNodes.toCreate, response.webhook_trigger_node_list ?? [], existingIdsByType('webhook-trigger'));
    mapByNewIds(nodeDiff.telegramNodes.toCreate, response.telegram_trigger_node_list ?? [], existingIdsByType('telegram-trigger'));
    mapByNewIds(nodeDiff.decisionTableNodes.toCreate, response.decision_table_node_list ?? [], existingIdsByType('decision-table'));
    ...
    mapByNewIds(nodeDiff.scheduleNodes.toCreate, response.schedule_trigger_node_list ?? [], existingIdsByType('schedule-trigger'));
    mapByNewIds(nodeDiff.classificationDecisionTableNodes.toCreate, response.classification_decision_table_node_list ?? [], existingIdsByType('classification-decision-table'));
    return mapping;
}
